...
+ **3、YUV422 420 444对于硬件实现有什么影响吗？**
  + 首先要理解YUV444\422\420的采样方式，只有444的采样单元是8x8，包含1个Y通道8x8单元，1个Cr 8x8单元以及1个Cb 8x8单元。422\420的采样单元是16x16，其中对于422来说，包含4个Y通道8x8单元，2个Cr 8x8单元以及2个Cb 8x8单元；对于420来说，包含4个Y通道8x8单元，1个Cr 8x8单元以及1个Cb 8x8单元。为了得到降采样的Cb Cr单元需要特殊降采样，这对于硬件实现不太方便。
  + 另一方面，对于444来说，只需要缓存8行就可以开始编码了；而对于422/420来说，则需要缓存16行，对于硬件实现来说消耗较大。例如，对于1920x1080来说，RGB888 24bit编码，缓存8行需要1920x24x8=368Kb，对于大多数FPGA来说还可以接受；而缓存16行的话，就需要736Kb，这对于很多FPGA来说就很难吃得消了，如果套上DDR作为帧缓存，每次按一个数据单元进行读取的话，可以极大缓解片内存储的压力，但失去了设计的灵活性和可移植性了。
